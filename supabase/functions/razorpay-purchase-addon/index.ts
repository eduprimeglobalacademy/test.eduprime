...
const KIND_LABEL: Record<string, string> = {
  extra_teachers: 'Extra teacher seat',
  extra_active_tests: 'Extra active test slot',
}
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: org } = await supabaseAdmin.from('organizations').select('plan_id').eq('id', adminUser.org_id).single()
    ...
    const addon = await razorpay(`/subscriptions/${subscription.razorpay_subscription_id}/addons`, {
      method: 'POST',
      body: JSON.stringify({
        item: { name: KIND_LABEL[kind], amount: unitPrice * 100, currency: 'INR' },
        quantity,
      }),
    })
